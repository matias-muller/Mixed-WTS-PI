...
% b = Num;
L = 44;%11; %length of the orginal filter
a = [1, -0.9854, 0.8187];
ahat = [1 zeros(1,L-1)];
b = [0, 0.2155, 0.2012];
% Filter gains
...
for n = 1:Nexp
    ...
    for t = 1:T
            ...
            e_wts = sigma*randn(N,1);
            y_wts = filter(b,a,u_wts) + e_wts;
...
%             X_im = mu_im + randn(K,1)*sigma./sqrt(2*p_wts);
            ...
            if( t < T_lim )
                ...
                % Perform experiment
                y_mixed = filter(b,a,u_mixed) + e_wts;
