...
% Metadata
K = 50;             % Number of arms {3, 10, 17, 24,... 7*L+3}
T = 1e2;            % Number iterative experiments
M = 1000;           % Number of MC samples in WTS
T_lim = 25;
Nexp = 1e3;         % Number of repetitions to average out
...
LL = 45;% floor(sqrt(2*N*T/log(2*N*T))); % length of LS FIR filter
a =  [1, -0.21969, 0.910569, -0.207156, 0.692948];%[1, -0.9854, 0.8187]; %;
ahat = [1 zeros(1,LL-1)];
b = [0, 0, 0, 0.2155, 0.2012]; %[0, 0.2155, 0.2012]; %
